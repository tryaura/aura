...
export const skl004 = defineCheck({
  ...
  explain:
    "Two skill entries in one application must not expose the same invocation name. Otherwise the application cannot offer a stable, unambiguous invocation target.\n\nRename a skill and its directory together, or remove the entry the application should not load.",
...
function detectInvocationCollisions(model: WorkspaceModel): readonly DetectedFinding[] {
  ...
  for (const group of groups.values()) {
    ...
    findings.push({
      ...
      metadata: { appId: first.appId, invocationName },
    });
  }
  return findings;
}
