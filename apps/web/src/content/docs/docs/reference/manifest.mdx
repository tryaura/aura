...
Think of the manifest as two lists:

- **Desired state** says which applications, skills, and MCP servers Aura should keep configured.
- **Ownership** says which exact entries Aura wrote last time, so it can update its own work without
  deleting neighboring user-authored configuration.

:::caution[Let Aura write the manifest]
Use `aura setup` and `aura check --fix` for normal changes. Manual edits bypass interactive review
and can put Aura into read-only mode; preserve a copy before repairing a damaged manifest.
:::
...
declared once per application: a second entry claiming it for an application that already has it
makes the manifest read-only, because convergence would otherwise pick a winner by position. Every
entry names at least one application. Aura also refuses recognized credential literals in
...
<CardGrid>
  <LinkCard
    title="Run setup safely"
    href="/docs/guides/setup/"
    description="Let the wizard write the manifest through one reviewed, reversible plan."
  />
  <LinkCard
    title="Read the team preset schema"
    href="/docs/reference/team-preset/"
    description="See which layer supplies each value the manifest finally records."
  />
  <LinkCard
    title="Troubleshoot a read-only manifest"
    href="/docs/troubleshooting/#the-manifest-is-read-only"
    description="Repair invalid JSON, an unsupported schema, or duplicate ownership."
  />
</CardGrid>
