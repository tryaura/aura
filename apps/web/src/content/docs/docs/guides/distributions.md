...
Eligible commands emit one or more events: `check-run` (apps detected, every executed check's state and finding counts, duration, exit code), an additional `fix-run` when fixing was requested, `setup-run` (how the run ended plus privacy-safe final choices once setup produced a plan), and `undo-run`. Help, explanations, and usage rejections emit no event. Events are delivered together in one batched flush as the process exits.

A `setup-run`'s `actions` reports only the categories the run actually put on screen. A category the wizard never asked about — because its step did not run, or ran with an empty catalog — is absent rather than empty, so an empty list always means the user was asked and chose nothing. Read the two apart before treating either as an adoption rate.

Payloads contain distribution-owned IDs, fixed-vocabulary actions and states, counts, booleans, and durations. They never contain file paths, file contents, finding messages, display names, environment values, detected application version strings, instruction source or duplicate choices, private skill-source approvals, custom MCP server names or transports, or externally sourced skill metadata. External skills and custom MCP servers are represented by counts.
