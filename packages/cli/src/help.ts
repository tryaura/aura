...
export function renderSetupHelp(branding: CliBranding, addKinds: readonly string[]): string {
  ...
  return renderHelpScreen(
    ...
    [
      // --yes takes the proposed default, and wherever the scan finds anything to combine that
      // default is to consolidate — the answer the action menu recommends. So the screen has to say
      // that files move before someone finds out afterwards.
