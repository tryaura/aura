...
describe("aura check --fix", () => {
  it("atomically wires Claude Code and Codex and is idempotent", async () => {
    ...
    expect(first.stdout).toContain("✓ 21 checks passed · 3 applications detected");
  ...
  it("leaves a real Codex instruction file byte-for-byte intact", async () => {
    ...
    expect(result.exitCode).toBe(0);
    // The report wraps prose at spaces, so the sentence is matched with its layout flattened back.
    expect(result.stdout.replaceAll(/\s+/gu, " ")).toContain(
      "Run `aura setup` and choose instruction consolidation",
    );
